...
int countOfZeroDataValues (NodePtr rootp)
{
	...
	if (rootp) {
		if ( rootp->datum == 0 ) {
			return(1 + countOfZeroDataValues(rootp->left) + countOfZeroDataValues(rootp->right));
		}
		else {
			return(0 + countOfZeroDataValues(rootp->left) + countOfZeroDataValues(rootp->right));
		}
	}
	
   	  // A "dummy instruction" that lets the program compile.
...
int height (NodePtr rootp)
{
	...
	if (!rootp)
		...
		leftHeight = height(rootp->left);
		rightHeight = height(rootp->right);
